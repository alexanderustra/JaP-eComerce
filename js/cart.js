document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSummary() {
    ...
    document.getElementById(
      "shipping-amount"
    ).textContent = `${flatShippingCost.toFixed(2)} USD`;
    document.getElementById(
      "total-amount"
    ).textContent = `${totalAmount.toFixed(2)} USD`;
  }
  ...
  const premiumRadioButtons = document.querySelectorAll(
    'input[name="premium"]'
  );
  ...
  function updateTotal() {
    ...
    document.getElementById(
      "shipping-amount"
    ).textContent = `${shippingCost.toFixed(2)} USD`;
    document.getElementById(
      "total-amount"
    ).textContent = `${totalAmount.toFixed(2)} USD`;
  }
    ...
    deleteBtn.addEventListener("click", () => {
      ...
      fetch(`http://localhost:3000/eliminar-del-carrito/${productId}`, {
        ...
        .then((response) => {
          if (!response.ok) {
            throw new Error(
              `Error al eliminar el producto del carrito: ${response.status}`
            );
          }
  ...
  function isBankAccountValid() {
    ...
    return Array.from(accountInputs).every(
  ...
  document
    .getElementById("finish-buy")
    .addEventListener("click", handleFinishBuyButtonClick);
  document
    .getElementById("payment-btn")
    .addEventListener("click", handlePaymentButtonClick);
  document
    .getElementById("close-modal")
    .addEventListener("click", () => hideModal("myModal"));
  document
    .getElementById("submitCreditCard")
    .addEventListener("click", handleSubmitCreditCardButtonClick);
  document
    .getElementById("wire-transfer")
    .addEventListener("click", handleWireTransferButtonClick);
  document
    .getElementById("submitBankAccount")
    .addEventListener("click", handleSubmitBankAccountButtonClick);
